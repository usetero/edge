...
pub const Prometheus = struct {
    /// Scrape budget from config.prometheus, applied to input and output (0 = unlimited).
    max_bytes_per_scrape: usize = 0,

    pub fn plan(self: *const Prometheus, _: service.PlanRequest) service.Outcome {
        return .{ .fetch_filtered = .{
            .upstream = .metrics,
            .max_input_bytes = self.max_bytes_per_scrape,
            .max_output_bytes = self.max_bytes_per_scrape,
        } };
    }
};

test "prometheus plans a filtered fetch with configured budgets" {
    const svc: Prometheus = .{ .max_bytes_per_scrape = 1024 };
    ...
    try std.testing.expectEqual(@as(usize, 1024), outcome.fetch_filtered.max_output_bytes);
}
